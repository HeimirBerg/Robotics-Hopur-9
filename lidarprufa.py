...
def _scan_worker():
    ...
    try:
        for scan in _lidar.iter_scans():
            if not _running:
                break
            # Filter out low quality and zero-distance readings, apply offset
            filtered = []
...
def start_lidar():
    ...
    t.start()
    time.sleep(2)  # Allow first scans to come in
    print("LiDAR ready")
...
def sense(last1=200, last2=200):
    ...
    # s0 and s1 reflect side clearance, limited by what's ahead
